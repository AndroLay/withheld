...
**As of 2026-09-01 19:19 UTC — 2026-09-02 early morning WITA.** Submission deadline: 2026-09-04 04:00
...
Each of these was checked by running it, on 2026-09-01, on Node 26.4.0. That version is the only one
...
- `pnpm test` — 110 tests, 110 pass, 0 fail.
...
- `pnpm build` — clean, 47 modules transformed. JS 250.50 kB raw / 77.97 kB gzipped, CSS 31.47 kB
  raw / 6.31 kB gzipped, `index.html` 0.99 kB / 0.51 kB gzipped.
...
- **The whole page renders, and that is now a test rather than a script I remembered to run.**
  `tests/render.test.mts` builds twenty-three renders through Vite's SSR module loader on every
...
- **The contract column renders in five states, and `App` renders whole.** With the worked example
  applied: 10 tool rows — the 9 that register plus the absent `confirm_release` — a read/write count
  taken from the registrations rather than typed above them, 5 "never crosses"
...
- **No inline `style` attribute in any of the twenty-three renders**, and every class name they ask for
...
- **Both directions of the stylesheet are now swept by a test.** Every `className` the page renders
  has a rule in `styles.css`, asserted by `tests/render.test.mts` over all twenty-three renders — the
...
- **The column is the tool surface, not a picture of it.** `toolSurfaceFacts()` builds the real nine
  ...
  printed payload `deepEqual` to what its tool returns, and no
  page-owned number in any of them. A tenth tool would appear on the page without the page being
  edited, and the absent one cannot be listed by that function at all — the `confirm_release` row is
...
drives it over the DevTools Protocol. The last run, on 2026-09-01 at 22:39 UTC against
Chrome/151.0.7922.137, reported **37 passed, 0 failed**. The record is
...
- **Every colour pair the page actually composes clears its WCAG threshold.** 423 visible text nodes
...
- **What a screen reader would be handed is now read, not assumed.** The full accessibility tree —
  1001 nodes — with every control and heading arriving named (32 of them), no unnamed node in any role
...
- **On a phone the contract column is a panel, and pressing it works.** At 420px the column is a
  closed `<details>` 110px tall — its summary and nothing else — with none of its ten tool rows
  visible; the summary is pressed, the column becomes 2538px with all ten visible, and pressed again
  it closes. At 1440px there is no panel at all: the column is a plain region beside the work. The
  probe asks `checkVisibility()` rather than measuring heights, because a closed `<details>` hides its
  contents with `content-visibility: hidden` and every row inside one still reports a box.
- 13 of 14 tab stops focus something; no element on the page carries an inline `style` attribute;
...
then reads the rendered page to see what moved. The last run, on 2026-09-01 at 22:40 UTC against
Chrome/151.0.7922.137 and `dist/` under `vite preview`, reported **17 passed, 0 failed**; the record is
...
- **The same write replayed at the revision it has already spent is refused `stale-revision`, and the
  page does not move a second time.** The fail-closed guard, observed from outside rather than called
  from a test.
...
- **No model has ever driven these tools.** The tools have now been invoked — by a DevTools Protocol
  ...
  | --- | --- | --- | --- |
  | the source | the functions do what they say | 110 tests, twenty-three renders | green |
  | the artefact in a browser | layout, contrast, the AX tree, CSP enforced, focus, console | `pnpm browser` — 37 checks | green |
  | the browser's registry | the API exists, nine tools registered | both scripts | green |
  | dispatch through it | a call from outside reaches the handler, and the page moves | `pnpm webmcp` — 17 checks | green |
...
| `src/domain/marks.ts` | the arithmetic, pure | 6 |
| `src/domain/session.ts` | authority: holds, refusals, receipts, release | 21 |
| `src/domain/views.ts` | paired projections, one per audience | 12 |
| `src/tools/agent-boundary.ts` | the fail-closed guard | 9 |
| `src/tools/webmcp.ts` | the nine tools, registration, and the two facts the page prints | 20 |
...
| `src/ui/Icon.tsx` | every glyph on the page, as inline SVG | via the twenty-three renders |
...
| `src/styles.css` | 2558 lines, no framework, no webfont | 8, and 7 more on the palette itself |
| `vite.config.ts` | build-only CSP injection | 1, as text |
| `scripts/browser-session.mjs` | 1009 lines: serves `dist/`, drives a headless Chromium over CDP, measures contrast and the AX tree, writes the evidence | it *is* the check — 37 |
| `scripts/webmcp-invoke.mjs` | invokes the nine tools through Chromium's `WebMCP` CDP domain and reads what the page did | it *is* the check — 17 |
...
`set_marking_emphasis`, `request_release`). There is no tenth, and the missing one is
`confirm_release`.
...
rather than of any one function: see `docs/GATE-W1.md`. `tests/render.test.mts` adds twenty-one the same
...
- **A local browser session.** The owner granted the permission on 2026-09-01 and it has been
  carried out; the last run reported 37 passed / 0 failed. See "Verified in a browser" above, and
...
- The page is not identical to the image it was built to. Eleven differences are deliberate and written
  ...
  invented ones, the tool list carries ten rows rather than seven, the release control in the top bar
...
- Six commits exist for this work: the first builds the package, the second records the browser
  ...
  states the claim, and the sixth measures contrast and the accessibility tree and folds the contract
  column on a phone. The monochrome rebuild is not among them — it is still in the working tree. None
