...
reports do not share that build: they describe source `09974722…` / build `3700f7c5…`, the build the
live URL served until the 18:02 republish put `84eee099…` there.
...
| `UNKNOWN` | Model-selected natural-language replay, and measurable impact | `evidence/natural-language-replay-blocked.json`, `docs/PREFLIGHT.md` | No valid observation exists in this task. No pass claim is made. The public repository, the final commit and the final manifest left this row on 2026-09-03: `AndroLay/withheld` reads back `main` at `9cce7d0a` and `gh-pages` at `15baf8f0` without credentials, and `evidence/manifest.json` is written. An MCP bridge, five host configs and two harnesses now exist for this row; none has been executed. |
